...
import { removeFriend } from "@/lib/server/social-server";
import { AdminConfigError } from "@/lib/firebase/admin";

export async function POST(req: Request) {
  try {
    const uid = await requireUidFromRequest(req);
    const body = (await req.json()) as { friendUid?: string };
    const friendUid = String(body.friendUid ?? "").trim();
    if (!friendUid) throw new HttpError(400, "معرّف الصديق مفقود.");
    ...
    return jsonError(400, "تعذر إزالة الصديق.");
  }
}
